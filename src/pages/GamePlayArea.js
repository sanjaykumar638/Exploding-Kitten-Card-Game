...
const GamePlayArea = props => {
  ...
  const revealCard = () => {
    ...
    if (poppedCard === 'SHUFFLE') {
      ...
      if (!cards.length) {
        ...
        const updatedDefusedCard = [...defuseCards]
      ...
      if (defuseCards.length) {
        if (!cards.length) {
          ...
          const updatedDefuseCards = [...defuseCards]
  ...
  const setOpenedCardSrc = () => {
    ...
    if (openedCard === 'SHUFFLE') {
      return shuffleCard
    }
    if (openedCard === 'DEFUSE') {
      return defuseCardImg
    }
    if (openedCard === 'CAT') {
      return catCard
    }
    if (openedCard === 'EXPLODE') {
      return explodeCard
    }
  }
      ...
      <section className="flex justify-center space-x-80 z-10 relative h-[80vh]  text-white">
        ...
        <section className="openedCard-container relative mb-4 w-48 h-64 flex-col items-center flex ">
          ...
          <div className="relative w-48 h-64 flex justify-center items-center">
            <div className="w-full bg-black h-60  top-0 absolute opacity-30" />
        ...
        <section className=" flex flex-col items-center">
          ...
          <div className=" max-h-60 h-60 min-w-[12rem]   relative ">
            <div className="w-full bg-black h-full top-0 absolute opacity-30" />

            {defuseCards.map((defuseCard, index) => {
              const right = index * 2
              const top = index * 12.7
              return (
                <div
                  // eslint-disable-next-line react/no-array-index-key
            ...
            {!defuseCards?.length && (
              <div className={`openedCard ${'card-zeroState'}`} />
            )}
          </div>
        </section>
      </section>
